...
class HrOverTime(models.Model):
    ...
    def submit_to_f(self):
       ...
       note='New Overtime needs action',            
       #Plans the activity 5 days in the future            
       date_deadline=datetime.datetime.today() + datetime.timedelta(days=0),           
       # Sets the current (logged in) user as responsible on the activity.           
       user_id=self.sudo()._get_responsible_for_approval().id or self.env.user.id)
